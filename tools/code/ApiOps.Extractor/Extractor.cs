...
internal class Extractor : BackgroundService
{
    ...
    public Extractor(IHostApplicationLifetime applicationLifetime, ILogger<Extractor> logger, IConfiguration configuration, AzureHttpClient azureHttpClient, NonAuthenticatedHttpClient nonAuthenticatedHttpClient)
    {
        ...
        tryGetResource = azureHttpClient.TryGetResourceAsJsonObject;
        getResource = azureHttpClient.GetResourceAsJsonObject;
        getResources = azureHttpClient.GetResourcesAsJsonObjects;
        serviceDirectory = GetServiceDirectory(configuration);
        serviceProviderUri = GetServiceProviderUri(configuration, azureHttpClient);
        serviceName = GetServiceName(configuration);
        apiSpecification = GetApiSpecification(configuration);
        configurationModel = configuration.Get<ConfigurationModel>();
    }
